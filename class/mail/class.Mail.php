<?php /* CLASSES $Id: class.Mail.php,v 1.1.1.1 2011/02/10 12:02:51 todorcvs Exp $ */
/**
 *	@package dotproject
 *	@subpackage utilites
*/
...
function Mail()
{
	...
	$this->username = isset($dPconfig['mail_user']) ? $dPconfig['mail_user'] : "";
	$this->password = isset($dPconfig['mail_pass']) ? $dPconfig['mail_pass'] : "";
	$this->defer = isset($dPconfig['mail_defer']) ? $dPconfig['mail_defer'] : "";
	$this->timeout = isset($dPconfig['mail_timeout']) ? $dPconfig['mail_timeout'] : 0;
	if (strlen($this->username)) { $this->auth = TRUE; } else { $this->auth = FALSE; }
}
...
function Subject( $subject, $charset='' ) {
	...
	$locale = isset($AppUI->user_locale) ? $AppUI->user_locale != 'en' : "en";
	
	
	if ( ( $locale != 'en' || ( $this->charset && $this->charset != 'us-ascii' && $this->charset != 'utf-8') ) && function_exists('imap_8bit')) {
		$subject = "=?".$this->charset."?Q?".
			str_replace("=\r\n","",imap_8bit($subject))."?=";		
	} elseif ( ( $locale != 'en' || ( $this->charset && $this->charset != 'us-ascii' && $this->charset != 'utf-8') ) && function_exists('quoted_printable')) {
		$subject = "=?".$this->charset."?Q?".
			str_replace("=\r\n","",quoted_printable($subject))."?=";
	}
...
//	$this->xheaders['To'] = implode( ", ", $this->sendto );
	...
	$this->xheaders["X-Mailer"] = "NBU Library Notifier 1.0";
...
function SMTPSend($to, $subject, $body, &$headers) {
	global $AppUI, $dPconfig, $classMail_Error;
	...
	if (! $this->socket) {
		...
		$classMail_Error = "Cannot connect to SMTP Host: $error_message ($error_number)";
		return false;
	}
	...
	if ($this->auth && $this->username) {
		...
		if (strpos($rcv, '235') !== 0) {
			...
			$classMail_Error = "Failed to login to SMTP server: $rcv";
			fclose($this->socket);
			return false;
		}
	}
	...
	if (substr($rcv,0,1) != '2') {
		...
		$classMail_Error = "432 Failed to send email: $rcv";
		return false;
	}
	foreach ($to as $to_address) {
		...
		if (substr($rcv,0,1) != '2') {
			//$AppUI->setMsg("Failed to send email: $rcv", UI_MSG_ERROR);
			$classMail_Error = "444 Failed to send email: $rcv";
			return false;
		}
	}
	...
	if (strpos($result, '250') === 0)
		...
		$classMail_Error = "463 Failed to send email: $result";
		return false;
	}
}

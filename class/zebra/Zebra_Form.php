 ...
 *  reset, button, file, password, radio buttons, checkboxes, hidden, captcha and date picker.
 *
 *  One additional note: this class is not a drag and drop utility - it is intended for coders who are comfortable with
 *  PHP, HTML, CSS and JavaScript - you will still have to build your forms when using this class but it saves a great
 *  deal of time and assures that your forms are secure and have a consistent look and feel throughout your projects!
 *
 ...
 *  @version    2.7.7 (last revision: January 18, 2012)
...
class Zebra_Form
{
    ...
    function add_error($error_block, $error_message)
    {
        ...
        // if the error block was not yet created
    ...
    function render($template = '', $return = false)
    {
            ...
            if (is_array($this->form_properties['clientside_validation']) && !empty($control->rules)) {
                ...
                foreach ($control->rules as $rule => $properties) {
                    ...
                    // (except those which do not have equivalents in JavaScript)
                        ...
                        // add the error message to the javascript object
                        $rules .= ($rules != '' ? ',' : '') . '"' . $rule . '":"' . addcslashes($properties[count($properties) - 1], '"') . '"';
        ...
        if ($template == '' || $template == '*horizontal' || $template == '*vertical') {
            ...
            foreach ($this->controls as $key=>$control) {
                ...
                // if control is a label that is to be placed inside another control
        ...
        // if $return argument was TRUE
        if ($return) {

            // return the result
            return $output;

        // if $return argument was FALSE
        } else {
                ...
                if ($form_is_valid) {
                    ...
                    if (isset($this->actions) && !empty($this->actions)) {
                        ...
                        foreach ($this->actions as $actions) {
                            ...
                            // if the task (method) could not be found
                            } else {

                                // trigger an error message
                                trigger_error('Method ' . $actions[0] . ' does not exist!', E_USER_ERROR);
                        ...
                        case 'custom':
                            ...
                            foreach ($rule_attributes as $custom_rule_attributes) {
                                ...
                                // if custom function doesn't exist
                                    ...
                                    // trigger an error message
                                    trigger_error('Function <strong>' . $custom_rule_attributes[0] . '()</strong> doesn\'t exist.', E_USER_ERROR);
                        ...
                        case 'length':
                            ...
                            if (
                                ...
                                (
                                    ...
                                    ($rule_attributes[1] > 0 && strlen($attribute['value']) > $rule_attributes[1])
    ...
    function _convert($control, $type, $jpeg_quality = 85, $preserve_original_file = false, $overwrite = false)
    {
        ...
        if (
            ...
            if ($type != $current_file_extension) {
                ...
                // if there was an error when resizing the image
                ...
                // if original file is not to be preserved
                if (!$preserve_original_file && !$overwrite) {

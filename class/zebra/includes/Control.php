...
class Zebra_Form_Control extends XSS_Clean
{
     ...
     *  -   <i>error_message</i> is the error message to be shown when rule is not obeyed
     *
     *  <b>This rule requires the prior inclusion of the {@link http://stefangabos.ro/php-libraries/zebra-image Zebra_Image}
     *  library!</b>
     *
     ...
     *  <i>file_types</i> as defined in <i>mimes.json</i> file - note that this is not a naive extension-based validation.
     *
     *  Available only for the {@link Zebra_Form_File file} control.
     *
     *  <code>
     ...
     *  $obj->set_rule(
     *       'filetype' => array(
     *          'xls, xlsx'                 /* allow only EXECEL files to be uploaded {@*}
     *          'error',                    /* variable to add the error message to {@*}
            ...
            foreach ($rules as $rule_name => $rule_properties) {
                ...
                switch ($rule_name) {
                    ...
                    case 'float':
                        ...
                        $this->set_attributes(array('onkeypress' => 'javascript:return $(\'#' . preg_replace('/\-/', '_', $this->form_properties['name']) . '\').data(\'Zebra_Form\').filter_input(\'' . $rule_name . '\', event' . ($rule_properties[0] != '' ? ', \'' . addcslashes($rule_properties[0], '\'') . '\'' : '') . ');'));

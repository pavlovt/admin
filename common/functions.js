...
function getXMLTagValue(recNode, tagName) {
	var node = recNode.getElementsByTagName(tagName)[0] ? recNode.getElementsByTagName(tagName)[0] : null;
	var value = (node && node.childNodes.length) ? node.childNodes[0].nodeValue : null;
	return value;
}

function getXMLTagValueWithIndex(recNode, tagName, i) {
	var node = recNode.getElementsByTagName(tagName)[i] ? recNode.getElementsByTagName(tagName)[i] : null;
	var value = (node && node.childNodes.length) ? node.childNodes[0].nodeValue : null;
	return value;
}
...
Array.prototype.inArrayGetIndex = function (value)
	// Returns index i if the passed value is found in the
